...
use App\Http\Controllers\daftr;
  ...
  $total=daftr::cartItem_steel_daftr();
?>
@if(Session::has('user'))
    {{ View::make('header')}}
...
<form method="POST" action="/maal_saalas/daftr_steel_entry">
    @csrf
                    ...
                    <div class="input-group"  >
                        ...
                        <div style="width: 140px">
                            <label style="float:right; font-size: larger; font-size: 16px; font-family: Jameel Noori; "><b>   دفتر کا نام  </b></label>
                                        ...
                                        <select class="form-control " name="daftr_no[]" style="border: 1px solid black;font-size:14px;">
                                           <option style="text-align: center;">سلیکٹ کریں</option>
                                           @foreach ($cols as $col)
                                           <option value="{{$col['daftr_no']}}">{{$col['daftr_no']}}</option>
                                           @endforeach
